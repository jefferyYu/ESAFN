...
class MMMGAN(nn.Module):
    def __init__(self, embedding_matrix, opt):
        super(MMMGAN, self).__init__()
        self.opt = opt
        ...
        if self.opt.tfn:
            ...
            self.text2text = nn.Linear(opt.hidden_dim*8, opt.hidden_dim)
            self.dense = nn.Linear(opt.hidden_dim*opt.hidden_dim, opt.polarities_dim)
        else:
            self.vis2text = nn.Linear(2048, opt.hidden_dim)
            self.dense = nn.Linear(opt.hidden_dim*9, opt.polarities_dim)
    ...
    def forward(self, inputs, visual_embeds_global, visual_embeds_mean, visual_embeds_att, att_mode):
        ...
        if self.opt.tfn:
            et = self.text2text(torch.tanh(feat))
